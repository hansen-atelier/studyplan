...
(function(){
  ...
  // Storage shim: prefer Claude's built-in window.storage (works when this
  // file is viewed inside Claude). Falls back to the browser's localStorage
  // (works when the file is downloaded and opened directly), so progress is
  // saved either way, every time the page reloads.
  ...
  async function storeSet(key, value){
    ...
    try { localStorage.setItem(key, value); } catch (e) { /* storage unavailable, skip */ }
  }
  ...
  async function loadWeek(week){
    const raw = await storeGet('week-' + week);
    weekState = raw ? JSON.parse(raw) : blankState();
    renderTracker();
  }

  async function saveWeek(){
    await storeSet('week-' + currentWeek, JSON.stringify(weekState));
  }
  ...
  async function setCurrentWeek(week, persist){
    ...
    renderPhaseUI(week);
    if(persist){
      await storeSet('current-week', String(week));
    }
    await loadWeek(week);
  }
